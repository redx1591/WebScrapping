...
def categoryAvechi():
    ...
    site = 'https://avechi.com'
    page_response = requests.get(site, headers={'User-Agent': 'Mozilla/5.0'})
    page_content = BeautifulSoup(page_response.content, "html.parser")

    category = page_content.find('ul',{"class":"groupmenu"}).findAll("li",{"class":"level0"})
    categories_urls = []

    for item in category:
        urlCategory =  item.find('a',{"class":"menu-link"}).get("href").replace('https://avechi.com/','')
...
#print(categoryAvechi())


def getAllPage():
    categories_urls = categoryAvechi()
    page = []
...
#print(scrapAvechi(origin=0))


"""INSERTION DES PRODUITS"""

produits = scrapAvechi(origin=0)
